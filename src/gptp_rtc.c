...
void set_rtc_time_adjusted(struct ptp_device *ptp, PtpTime * time,
			   PtpTime * entryTime)
{
	unsigned long flags;
	PtpTime timeNow;
...
void update_rtc_lock_detect(struct ptp_device *ptp)
{
	...
	int32_t unlockThreshSigned =
	    (int32_t) ptp->properties.unlockThreshNsec;
...
static void computeDelayRateRatio(struct ptp_device *ptp, uint32_t port)
{
	if (ptp->ports[port].initPdelayRespReceived == FALSE) {
		...
		ptp->ports[port].pdelayRespTxTimestampI =
		    ptp->ports[port].delayReqTxLocalTimestamp;
		ptp->ports[port].pdelayRespRxTimestampI =
		    ptp->ports[port].delayReqRxTimestamp;
	} else {
		PtpTime difference;
		PtpTime difference2;
...
void rtc_update_servo(struct ptp_device *ptp, uint32_t port)
{
	...
	if (ptp->properties.delayMechanism == PTP_DELAY_MECHANISM_E2E) {
		 ...
		 */
		if (ptp->ports[port].syncTimestampsValid
		    && ptp->ports[port].delayReqTimestampsValid) {
			PtpTime difference2;
			...
			ptp->ports[port].neighborPropDelay =
			    (-difference2.nanoseconds) + slaveOffset;
		 ...
		 */
		timestamp_difference(&ptp->ports[port].syncRxTimestamp,
	...
	if (slaveOffsetValid == PTP_RTC_OFFSET_VALID) {
		...
		if (ptp->masterRateRatioValid) {
			uint32_t scaledMasterRateRatio =
			    ptp->masterRateRatio >> 1;
			int32_t rateDiff =
			    (int32_t) (scaledMasterRateRatio -
				       ptp->prevBaseRtcIncrement);
			...
			if (((slaveOffset < 0)
			    ...
			    || ((slaveOffset > 0)
				&& (ptp->previousOffset < 0))) {
				uint64_t prevIntegral =
				    ptp->zeroCrossingIntegral;
		...
		if (ptp->acquiring == PTP_RTC_ACQUIRING) {
			if ((slaveOffset > ACQUIRE_THRESHOLD)
			    || (slaveOffset < -ACQUIRE_THRESHOLD)) {
				...
				if (servoCount >= 10) {
					...
					wordChunk =
					    (uint32_t) (accumulator >> 32);
					printk
					    ("Acquiring, P contribution = 0x%08X",
					     wordChunk);
		...
		if (ptp->acquiring == PTP_RTC_ACQUIRED) {
			 ...
			 */
			if ((slaveOffset > (8 * ACQUIRE_THRESHOLD))
			    || (slaveOffset < (8 * -ACQUIRE_THRESHOLD))) {
			...
			accumulator =
			    ((coefficient *
			      slaveOffsetExtended) >> COEFF_PRODUCT_SHIFT);
			...
			accumulator +=
			    ((coefficient *
			      ptp->integral) >> COEFF_PRODUCT_SHIFT);
			...
			accumulator +=
			    ((coefficient *
			      ptp->derivative) >> COEFF_PRODUCT_SHIFT);
		...
		if (ptp->rtcChangesAllowed) {
			...
			if ((ptp->prevAppliedRtcIncrement -
			     newRtcIncrement) < 0x200
			    || (newRtcIncrement -
				ptp->prevAppliedRtcIncrement) < 0x200) {
				newRtcIncrement =
				    (newRtcIncrement >> 1) +
				    (ptp->prevAppliedRtcIncrement >> 1);
			}
			ptp->rtcLastIncrementDelta =
			    newRtcIncrement - ptp->prevAppliedRtcIncrement;
			ptp->prevAppliedRtcIncrement = tempRtcIncrement;

			RtcIncrement newIncrement;
			newIncrement.mantissa =
			    (newRtcIncrement >> RTC_MANTISSA_SHIFT) &
			    RTC_MANTISSA_MASK;
			newIncrement.fraction =
			    (newRtcIncrement & RTC_FRACTION_MASK);
