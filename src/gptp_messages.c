			    ...
			    uint16_t headerFlags)
{
	...
	*wordOffset = 0;
	write_packet(txBuffer, wordOffset,
		     (ETH_HEADER_BYTES + messageLength -
		      TX_LENGTH_SUB(ptp)));
	...
	*wordOffset = TX_DATA_OFFSET(ptp);
	...
	packetWord =
	    (((uint32_t) portProperties->sourceMacAddress[2]) << 24);
	packetWord |=
	    (((uint32_t) portProperties->sourceMacAddress[3]) << 16);
	packetWord |=
	    (((uint32_t) portProperties->sourceMacAddress[4]) << 8);
	...
	write_packet(txBuffer, wordOffset, 0x00000000);
	packetWord =
	    (((uint32_t) ptp->properties.grandmasterIdentity[0]) << 8);
	packetWord |= ptp->properties.grandmasterIdentity[1];
	write_packet(txBuffer, wordOffset, packetWord);
	packetWord =
	    (((uint32_t) ptp->properties.grandmasterIdentity[2]) << 24);
	packetWord |=
	    (((uint32_t) ptp->properties.grandmasterIdentity[3]) << 16);
	packetWord |=
	    (((uint32_t) ptp->properties.grandmasterIdentity[4]) << 8);
	packetWord |= ptp->properties.grandmasterIdentity[5];
	write_packet(txBuffer, wordOffset, packetWord);
	packetWord =
	    (((uint32_t) ptp->properties.grandmasterIdentity[6]) << 24);
	packetWord |=
	    (((uint32_t) ptp->properties.grandmasterIdentity[7]) << 16);
	packetWord |= port + 1;
	write_packet(txBuffer, wordOffset, packetWord);
				   ...
				   PtpPriorityVector * pv)
{
	...
	init_ptp_header(ptp, port, txBuffer, &wordOffset, MSG_ANNOUNCE,
			PTP_ANNOUNCE_LENGTH + TLV_HEADER_LENGTH +
			PATH_TRACE_TLV_LENGTH(ptp->pathTraceLength),
	...
	write_packet(txBuffer, &wordOffset, packetWord);
	packetWord =
	    (((uint32_t) identity->offsetScaledLogVariance[0]) << 24);
	packetWord |=
	    (((uint32_t) identity->offsetScaledLogVariance[1]) << 16);
			 ...
			 uint8_t * packetBuffer)
{
	...
	bufferBase =
	    (bufferDirection ==
	     TRANSMITTED_PACKET) ? packetBuffer +
	    TX_DATA_OFFSET(ptp) : packetBuffer;
			  ...
			  uint8_t * rxBuffer, PtpTime * correctionField)
{
	...
	correctionField->nanoseconds =
	    (uint32_t) (rawField >> CORRECTION_FRACTION_BITS);
}
			       ...
			       Integer96 * lastGmPhaseChange)
{
	...
	lastGmPhaseChange->upper =
	    ((read_packet(rxBuffer, &wordOffset) & 0x0000FFFF) << 16);
	packetWord = read_packet(rxBuffer, &wordOffset);
	...
	lastGmPhaseChange->lower |=
	    ((read_packet(rxBuffer, &wordOffset) & 0xFFFF0000) >> 16);
}
...
uint16_t get_port_number(const uint8_t * portNumber)
{
	...
	return ((uint16_t)
		((((uint16_t) portNumber[0]) << 8) | portNumber[1]));
}
...
uint16_t get_steps_removed(const uint8_t * stepsRemoved)
{
	/* Fetch the big-endian packed value */
	return ((uint16_t)
		((((uint16_t) stepsRemoved[0]) << 8) | stepsRemoved[1]));
}
					...
					offsetScaledLogVariance)
{
	/* Fetch the big-endian packed value */
	return ((uint16_t)
		((((uint16_t) offsetScaledLogVariance[0]) << 8) |
				   ...
				   uint8_t * requestPortId)
{
	...
	packetWord &= 0xFFFF0000;
	packetWord |=
	    ((((uint32_t) requestPortId[0]) << 8) | requestPortId[1]);
	wordOffset -= BYTES_PER_WORD;
	write_packet(bufferBase, &wordOffset, packetWord);
	packetWord =
	    ((((uint32_t) requestPortId[2]) << 24) |
	     (((uint32_t) requestPortId[3]) << 16) |
	     (((uint32_t) requestPortId[4]) << 8) | requestPortId[5]);
	write_packet(bufferBase, &wordOffset, packetWord);
	packetWord =
	    ((((uint32_t) requestPortId[6]) << 24) |
	     (((uint32_t) requestPortId[7]) << 16) |
...
void transmit_announce(struct ptp_device *ptp, uint32_t port)
{
	...
	txBuffer = get_output_buffer(ptp, port, PTP_TX_ANNOUNCE_BUFFER);
	set_sequence_id(ptp, port, txBuffer,
...
void transmit_fup(struct ptp_device *ptp, uint32_t port)
{
	...
	set_sequence_id(ptp, port, txFupBuffer,
			get_sequence_id(ptp, port, TRANSMITTED_PACKET,
					txSyncBuffer));
	set_log_message_interval(ptp, port, txFupBuffer,
				 ptp->ports[port].currentLogSyncInterval);

	if (ptp->ports[port].syncTxLocalTimestampValid) {
		PtpTime residency;
		PtpTime correction;
		timestamp_difference(&ptp->ports[port].syncTxTimestamp,
	...
	set_cumulative_scaled_rate_offset_field(ptp, txFupBuffer,
						rateRatio);
...
void transmit_delay_request(struct ptp_device *ptp, uint32_t port)
{
	...
	txBuffer = get_output_buffer(ptp, port, PTP_TX_DELAY_REQ_BUFFER);
	set_sequence_id(ptp, port, txBuffer,
			ptp->ports[port].delayReqSequenceId++);
			     ...
			     uint8_t * requestRxBuffer)
{
	...
	get_source_port_id(ptp, port, RECEIVED_PACKET, requestRxBuffer,
			   requestPortId);
	delayReqSequenceId =
	    get_sequence_id(ptp, port, RECEIVED_PACKET, requestRxBuffer);
...
void transmit_pdelay_response_fup(struct ptp_device *ptp, uint32_t port)
{
	...
	txFupBuffer =
	    get_output_buffer(ptp, port, PTP_TX_PDELAY_RESP_FUP_BUFFER);
	txRespBuffer =
	    get_output_buffer(ptp, port, PTP_TX_PDELAY_RESP_BUFFER);
		      ...
		      uint8_t * rxBuffer, PtpPriorityVector * pv)
{
	...
	packetWord = read_packet(rxBuffer, &wordOffset);
	pv->rootSystemIdentity.offsetScaledLogVariance[0] =
	    (uint8_t) (packetWord >> 24);
	pv->rootSystemIdentity.offsetScaledLogVariance[1] =
	    (uint8_t) (packetWord >> 16);
...
void copy_ptp_properties(PtpProperties * to, PtpProperties * from)
{
	...
	to->grandmasterClockQuality.clockClass =
	    from->grandmasterClockQuality.clockClass;
	to->grandmasterClockQuality.clockAccuracy =
	    from->grandmasterClockQuality.clockAccuracy;
	to->grandmasterClockQuality.offsetScaledLogVariance =
	    from->grandmasterClockQuality.offsetScaledLogVariance;
	to->grandmasterPriority2 = from->grandmasterPriority2;
	for (byteIndex = 0; byteIndex < PTP_CLOCK_IDENTITY_BYTES;
	     byteIndex++) {
		to->grandmasterIdentity[byteIndex] =
		    from->grandmasterIdentity[byteIndex];
	}
			      ...
			      PtpPortProperties * from)
{
	...
	for (byteIndex = 0; byteIndex < MAC_ADDRESS_BYTES; byteIndex++) {
		to->sourceMacAddress[byteIndex] =
		    from->sourceMacAddress[byteIndex];
	}
			       ...
			       const uint8_t * clockIdentityB)
{
	...
	for (byteIndex = 0; byteIndex < PTP_CLOCK_IDENTITY_BYTES;
		...
		if (clockIdentityA[byteIndex] < clockIdentityB[byteIndex]) {
			comparisonResult = -1;
			break;
		} else if (clockIdentityA[byteIndex] >
			   clockIdentityB[byteIndex]) {
			comparisonResult = 1;
			break;
		}
	}
