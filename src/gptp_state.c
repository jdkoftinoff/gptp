...
/* Function Prototypes */
uint8_t *get_output_buffer(struct ptp_device *ptp, uint32_t port,
			   uint32_t bufType);
...
void gptp_timer_state_task(unsigned long data)
{
	...
	for (i = 0; i < ptp->numPorts; i++) {
		...
		if (ptp->ports[i].multiplePdelayTimer > 0) {
			if (ptp->ports[i].multiplePdelayTimer >=
			    timerTicks) {
				ptp->ports[i].multiplePdelayTimer -=
				    timerTicks;
			} else {
			...
			if (ptp->ports[i].multiplePdelayTimer == 0) {
				printk
				    ("Re-enabled ptp on port %d after 5 minutes\n",
				     i + 1);
		...
		if (ptp->ports[i].multiplePdelayTimer == 0) {
			...
			case PTP_MASTER:
				...
				ptp->ports[i].announceCounter +=
				    timerTicks;
				if (ptp->ports[i].announceCounter >=
				    ANNOUNCE_INTERVAL_TICKS(ptp, i)) {
				...
				if (ptp->gmPresent) {
					ptp->ports[i].syncCounter +=
					    timerTicks;
					if (ptp->ports[i].syncCounter >=
					    SYNC_INTERVAL_TICKS(ptp, i)) {
						ptp->ports[i].syncCounter =
						    0;
			...
			case PTP_SLAVE:
				{
					...
					if (++ptp->slaveDebugCounter >=
					    timeoutTicks) {
						ptp->slaveDebugCounter = 0;

						printk
						    ("PTP increment: 0x%08X\n",
						     ptp_get_increment());
					}
#endif
	 ...
	 * to monitor.
	 */
	if (++ptp->heartbeatCounter >=
	    (HEARTBEAT_INTERVAL / PTP_TIMER_TICK_MS)) {
	...
	if (ptp->rtcLastLockState != ptp->rtcLockState)
		ptp_events_tx_rtc_change(ptp);
	ptp->rtcLastLockState = ptp->rtcLockState;
}

/* Processes a newly-received ANNOUNCE packet for the passed instance */
static void process_rx_announce(struct ptp_device *ptp, uint32_t port,
				uint8_t * rxBuffer)
{
...
static void process_rx_sync(struct ptp_device *ptp, uint32_t port,
			    uint8_t * rxBuffer)
{
	unsigned long flags;
	...
	if ((ptp->ports[port].selectedRole == PTP_SLAVE)
	    && (0 ==
		memcmp(&rxIdentity, &ptp->gmPriority->sourcePortIdentity,
		       sizeof(PtpPortIdentity)))) {
		PtpTime tempTimestamp;
		PtpTime correctionField;
		PtpTime correctedTimestamp;

		ptp->ports[port].syncReceiptTimeoutTime =
		    SYNC_INTERVAL_TICKS(ptp,
					port) *
		    ptp->ports[port].syncReceiptTimeout;
		 ...
		 */
		get_hardware_timestamp(ptp, port, RECEIVED_PACKET,
				       rxBuffer, &tempTimestamp);

		get_correction_field(ptp, port, rxBuffer,
				     &correctionField);
		timestamp_difference(&tempTimestamp, &correctionField,
				     &correctedTimestamp);

		preempt_disable();
		spin_lock_irqsave(&ptp->mutex, flags);
		timestamp_copy(&ptp->ports[port].syncRxTimestampTemp,
			       &correctedTimestamp);
#ifdef DEBUG_MISSED_TIMESTAMP
		if (((ptp->ports[port].syncSequenceId + 1) & 0xffff) !=
		    get_sequence_id(ptp, port, RECEIVED_PACKET,
				    rxBuffer)) {
			printk("missed sync %d (%d)\r\n",
			       ptp->ports[port].syncSequenceId,
			       get_sequence_id(ptp, port, RECEIVED_PACKET,
					       rxBuffer) -
			       (ptp->ports[port].syncSequenceId + 1));
		}
#endif
		ptp->ports[port].syncSequenceId =
		...
		if (ptp->ports[port].selectedRole == PTP_SLAVE) {
			...
			PtpTime linkDelay;
			get_local_hardware_timestamp(ptp, port,
						     RECEIVED_PACKET,
						     rxBuffer,
						     &syncRxTimestamp);
...
void gptp_signal_gm_change(struct ptp_device *ptp)
{
...
static void process_rx_fup(struct ptp_device *ptp, uint32_t port,
			   uint8_t * rxBuffer)
{
	unsigned long flags;
	PtpPortIdentity rxIdentity;
	...
	if ((ptp->ports[port].selectedRole == PTP_SLAVE)
		 ...
		 */
		get_timestamp(ptp, port, RECEIVED_PACKET, rxBuffer,
			      &syncTxTimestamp);
		...
		timestamp_sum(&syncTxTimestamp, &correctionField,
			      &correctedTimestamp);

		preempt_disable();
		spin_lock_irqsave(&ptp->mutex, flags);
		timestamp_copy(&ptp->ports[port].syncRxTimestamp,
			       &ptp->ports[port].syncRxTimestampTemp);
		timestamp_copy(&ptp->ports[port].syncTxTimestamp,
			       &correctedTimestamp);
		...
		ptp->ports[port].cumulativeScaledRateOffset =
		    get_cumulative_scaled_rate_offset_field(rxBuffer);

		/* Treat GM phase change just like a GM change */
		if (get_gm_time_base_indicator_field(rxBuffer) !=
		    ptp->lastGmTimeBaseIndicator) {
			printk("GM Phase Change\n");
			gptp_signal_gm_change(ptp);

			ptp->lastGmTimeBaseIndicator =
			    get_gm_time_base_indicator_field(rxBuffer);
			get_gm_phase_change_field(rxBuffer,
						  &ptp->lastGmPhaseChange);
			ptp->lastGmFreqChange =
			    get_gm_freq_change_field(rxBuffer);
		}
		...
		timestamp_difference(&ptp->ports[port].syncRxTimestampTemp,
				     &correctedTimestamp, &difference);
		...
		if ((absDifference.secondsUpper > 0)
			...
			printk
			    ("Sync Tx: %08X%08X.%08X (corrected: %08X%08X.%08X\n",
			     ...
			     syncTxTimestamp.secondsLower,
			     syncTxTimestamp.nanoseconds,
			     correctedTimestamp.secondsUpper,
			     correctedTimestamp.secondsLower,
			     correctedTimestamp.nanoseconds);
...
static void process_rx_signaling(struct ptp_device *ptp, uint32_t port,
				 uint8_t * rxBuffer)
{
...
static void process_rx_delay_req(struct ptp_device *ptp, uint32_t port,
				 uint8_t * rxBuffer)
{
...
static void process_rx_delay_resp(struct ptp_device *ptp, uint32_t port,
				  uint8_t * rxBuffer)
{
	unsigned long flags;
...
static void process_rx_pdelay_req(struct ptp_device *ptp, uint32_t port,
				  uint8_t * rxBuffer)
{
	...
	if (0 !=
		...
		uint16_t rxPortNumber =
		    get_port_number(rxIdentity.portNumber);
		printk
		    ("Disabling AS on ports %d and %d due to receipt of our own pdelay.\n",
		     port + 1, rxPortNumber);
...
static void process_rx_pdelay_resp(struct ptp_device *ptp, uint32_t port,
				   uint8_t * rxBuffer)
{
...
static void process_rx_pdelay_resp_fup(struct ptp_device *ptp,
...
void gptp_rx_state_task(unsigned long data)
{
	struct ptp_device *ptp = (struct ptp_device *) data;
	int i;
...
void process_rx_buffer(struct ptp_device *ptp, int port, uint8_t * buffer)
{
	if (TRANSPORT_PTP == get_transport_specific(ptp, port, buffer)) {
		...
		printk
		    ("WARNING: Unrecognized transportSpecific rejected\n");
}

/* Tasklet function for PTP Tx packets */
void gptp_tx_state_task(unsigned long data)
{
	struct ptp_device *ptp = (struct ptp_device *) data;
			...
			if ((pendingTxFlags & bufferMask) !=
				...
				case PTP_TX_DELAY_REQ_BUFFER:{
						...
						txBuffer =
						    get_output_buffer(ptp,
								      i,
								      PTP_TX_DELAY_REQ_BUFFER);
						get_hardware_timestamp(ptp,
								       i,
								       TRANSMITTED_PACKET,
								       txBuffer,
								       &ptp->
...
void init_state_machines(struct ptp_device *ptp)
{
...
#ifndef CONFIG_GPTP_NO_TASKLET
	tasklet_init(&ptp->timerTasklet, &gptp_timer_state_task,
		     (unsigned long) ptp);
#endif
	ptp->heartbeatCounter = 0;
	...
	for (i = 0; i < ptp->numPorts; i++) {
...
#ifdef CONFIG_OF
		interfaceDev =
		    of_find_device_by_node(pPort->interfaceNode);
		ndev =
		...
		pPort->portAnnounceInformation_State =
		    PortAnnounceInformation_BEGIN;
	}
...
#ifndef CONFIG_GPTP_NO_TASKLET
	tasklet_init(&ptp->rxTasklet, &gptp_rx_state_task,
		     (unsigned long) ptp);
#endif
	...
	tasklet_init(&ptp->txTasklet, &gptp_tx_state_task,
		     (unsigned long) ptp);
#endif
}
