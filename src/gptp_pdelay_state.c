/*
 *  linux/drivers/net/gptp_pdelay_state.c
 *
...
static void computePdelayRateRatio(struct ptp_device *ptp, uint32_t port)
{
	if (ptp->ports[port].initPdelayRespReceived == FALSE) {
		...
		ptp->ports[port].neighborRateRatioValid =
		    ptp->ports[port].isMeasuringDelay;
		ptp->ports[port].pdelayRespTxTimestampI =
		    ptp->ports[port].pdelayRespTxTimestamp;
		ptp->ports[port].pdelayRespRxTimestampI =
		    ptp->ports[port].pdelayRespRxTimestamp;
	} else {
		PtpTime difference;
		PtpTime difference2;
		...
		if ((difference.secondsUpper & 0x80000000) |
			...
			nsResponder =
			    ((uint64_t) difference.secondsLower) *
			    1000000000ULL +
			    (uint64_t) difference.nanoseconds;
			nsRequester =
			    ((uint64_t) difference2.secondsLower) *
			    1000000000ULL +
			    (uint64_t) difference2.nanoseconds;

			for (shift = 0; shift < 31; shift++) {
				if (nsResponder & (1ULL << (63 - shift)))
					break;
			}
			...
			if ((nsRequester >> (31 - shift)) != 0) {
				rateRatio =
				    (nsResponder << shift) /
				    (nsRequester >> (31 - shift));
				ptp->ports[port].neighborRateRatio =
				    (uint32_t) rateRatio;
			...
			printk
			    ("Responder delta: %08X%08X.%08X (%llu ns)\n",
			     difference.secondsUpper,
			     difference.secondsLower,
			     difference.nanoseconds, nsResponder);
			printk
			    ("Requester delta: %08X%08X.%08X (%llu ns)\n",
			     difference2.secondsUpper,
			     difference2.secondsLower,
			     difference2.nanoseconds, nsRequester);
...
static void computePropTime(struct ptp_device *ptp, uint32_t port)
{
	if (ptp->ports[port].neighborRateRatioValid) {
		...
		nsResponder =
		    ...
		    (uint64_t) difference.nanoseconds;
		nsRequester =
		    ((uint64_t) difference2.secondsLower) * 1000000000ULL +
		    (uint64_t) difference2.nanoseconds;

		ptp->ports[port].neighborPropDelay =
		    (((((uint64_t) ptp->ports[port].neighborRateRatio) *
		       nsRequester) >> 31) - nsResponder) >> 1;

#ifdef PATH_DELAY_DEBUG
		printk("Responder delta: %08X%08X.%08X (%llu ns)\n",
		       difference.secondsUpper, difference.secondsLower,
		       difference.nanoseconds, nsResponder);
		printk("Requester delta: %08X%08X.%08X (%llu ns)\n",
		       difference2.secondsUpper, difference2.secondsLower,
		       difference2.nanoseconds, nsRequester);
...
static void MDPdelayReq_StateMachine_SetState(struct ptp_device *ptp,
					      uint32_t port,
					      MDPdelayReq_State_t newState)
{
#ifdef PATH_DELAY_DEBUG
	...
	case MDPdelayReq_LOST_RESPONSE:
		...
		if (port == 0)
			printk("lostResponses %d\n",
			       ptp->ports[port].lostResponses);
#endif
		if (ptp->ports[port].lostResponses >
		    ptp->ports[port].allowedLostResponses) {
			ptp->ports[port].asCapable = FALSE;
#ifdef DEBUG_OUTPUT
			if (port == 0)
				printk("asCapable %d\n",
	...
	case MDPdelayReq_WAITING_FOR_PDELAY_RESP_FOLLOW_UP:
		...
		get_timestamp(ptp, port, RECEIVED_PACKET,
			      ptp->ports[port].rcvdPdelayRespPtr,
	...
	case MDPdelayReq_WAITING_FOR_PDELAY_INTERVAL_TIMER:
		...
		get_timestamp(ptp, port, RECEIVED_PACKET,
			      ptp->ports[port].rcvdPdelayRespFollowUpPtr,
			      &ptp->ports[port].pdelayRespTxTimestamp);
		...
		if (ptp->ports[port].isMeasuringDelay != FALSE) {
			...
			if ((ptp->ports[port].neighborPropDelay <=
			     ptp->ports[port].neighborPropDelayThresh)
			    &&
			    (compare_clock_identity
			     (ptp->ports[port].lastRxRespSourcePortId,
			      ptp->properties.grandmasterIdentity) != 0)
			    && ptp->ports[port].neighborRateRatioValid) {
...
//    ptp->ports[port].pttPortEnabled, ptp->ports[port].pdelayIntervalTimer, PDELAY_REQ_INTERVAL_TICKS(ptp, port));
	...
	if (!ptp->ports[port].portEnabled
	    || !ptp->ports[port].pttPortEnabled) {
		if (ptp->ports[port].mdPdelayReq_State !=
		    MDPdelayReq_NOT_ENABLED) {
			/* Disabling the port immediately forces the state machine into the disabled state */
			MDPdelayReq_StateMachine_SetState(ptp, port,
							  MDPdelayReq_NOT_ENABLED);
		}
	} else {
		...
		do {
			...
			case MDPdelayReq_LOST_RESPONSE:
				MDPdelayReq_StateMachine_SetState(ptp,
								  port,
								  MDPdelayReq_RESET);
				break;
			...
			case MDPdelayReq_SEND_PDELAY_REQ:
								     ...
								     port))
				{
					...
					MDPdelayReq_StateMachine_SetState
					    (ptp, port, MDPdelayReq_RESET);
#ifdef PATH_DELAY_DEBUG
					printk
					    ("ptp: We didn't see a timestamp for some reason on port %d\n",
					     port);
#endif
			...
			case MDPdelayReq_WAITING_FOR_PDELAY_RESP:
				if ((ptp->ports[port].pdelayIntervalTimer >= PDELAY_REQ_INTERVAL_TICKS(ptp, port))) {	/* Timeout */
#ifdef DEBUG_OUTPUT
					if (port == 0)
				...
				} else if (ptp->ports[port].rcvdPdelayResp) {	// Response received
			...
			case MDPdelayReq_WAITING_FOR_PDELAY_RESP_FOLLOW_UP:
				if (ptp->ports[port].pdelayIntervalTimer >= PDELAY_REQ_INTERVAL_TICKS(ptp, port)) {	// Timeout
					...
					if (port == 0)
						printk
						    ("Timeout WAITING_FOR_PDELAY_RESP_FOLLOW_UP\n");
#endif
					MDPdelayReq_StateMachine_SetState
					    (ptp, port,
					     MDPdelayReq_LOST_RESPONSE);
				...
				if (ptp->ports[port].rcvdPdelayResp) {	// Response received
					ptp->ports[port].rcvdPdelayResp =
					    FALSE;
#ifdef DEBUG_OUTPUT
					if (port == 0)
						printk
						    ("Response received WAITING_FOR_PDELAY_RESP_FOLLOW_UP\n");
#endif
					if ((compare_port_ids(rxRequestingPortId, txRequestingPortId) != 0) ||	// If the responder says it's not responding to us
					    (compare_port_ids(rxRespSourcePortId, txRequestingPortId) == 0) ||	// If we're responding to ourself
					    (rxSequenceId != txSequenceId))	// If the sequence number does not match
					{	// Non-matching response
						...
						if (port == 0)
							printk
							    ("Non-matching response received WAITING_FOR_PDELAY_RESP_FOLLOW_UP\n");
#endif
						// Ignore this resp.  Processing it is causing rapid fire req's.
						//MDPdelayReq_StateMachine_SetState(ptp, port, MDPdelayReq_LOST_RESPONSE);
					} else {	// Matching duplicate response
							...
							if (ptp->ports[port].multiplePdelayResponses > 1) {	/* We've had more than one multiple response occurrence */
								if ((ptp->ports[port].prevPdelayReqSequenceId + 1) == txSequenceId) {	/* The occurrences are on consecutive reqs */
									if (ptp->ports[port].multiplePdelayResponses >= 3) {	/* We've had too many multiple responses, disable */
										printk
										    ("Disabling AS on port %d due to multiple pdelay responses (%d %d).\n",
										     port
										     +
										     1,
										     ptp->
				...
				if (ptp->ports[port].rcvdPdelayRespFollowUp) {	// Received follow up
				...
				break;
			case MDPdelayReq_WAITING_FOR_PDELAY_INTERVAL_TIMER:
				if (ptp->ports[port].pdelayIntervalTimer >=
				    PDELAY_REQ_INTERVAL_TICKS(ptp, port)) {
...
static void LinkDelaySyncIntervalSetting_StateMachine_SetState(struct
							       ptp_device
							       *ptp,
							       uint32_t
							       port,
							       ...
							       newState)
{
#ifdef PATH_DELAY_DEBUG
	printk
	    ("LinkDelaySyncIntervalSetting: Set State %d (port index %d)\n",
	     newState, port);
#endif
	...
	case LinkDelaySyncIntervalSetting_INITIALIZE:
		ptp->ports[port].currentLogPdelayReqInterval =
		    ptp->ports[port].initialLogPdelayReqInterval;
		ptp->ports[port].currentLogSyncInterval =
		    ptp->ports[port].initialLogSyncInterval;
	...
	case LinkDelaySyncIntervalSetting_SET_INTERVALS:
		{
			...
			uint32_t packetWord =
			    read_packet(ptp->ports[port].rcvdSignalingPtr,
					&offset);
			int8_t linkDelayInterval =
			    (int8_t) ((packetWord >> 24) & 0xFF);
			int8_t timeSyncInterval =
			    (int8_t) ((packetWord >> 16) & 0xFF);
			...
			ptp->ports[port].computeNeighborRateRatio =
			    flags & (1 << 0);
			ptp->ports[port].computeNeighborPropDelay =
			    flags & (1 << 1);
...
void LinkDelaySyncIntervalSetting_StateMachine(struct ptp_device *ptp,
					       uint32_t port)
{
	if (!ptp->ports[port].portEnabled
	    || !ptp->ports[port].pttPortEnabled) {
		if (ptp->ports[port].linkDelaySyncIntervalSetting_State !=
		    LinkDelaySyncIntervalSetting_NOT_ENABLED) {
			/* Disabling the port immediately forces the state machine into the disabled state */
			LinkDelaySyncIntervalSetting_StateMachine_SetState
			    (ptp, port,
			     LinkDelaySyncIntervalSetting_NOT_ENABLED);
		}
	} else {
